...
/*
    Purpose: The main lifting of the code
            ...
            5. Sends SNS notification to users
*/
function signAndSendRequestToES(singlejsonPayload)
{
    var excludeEventArray = ["DescribeTargetHealth"]
    if (excludeEventArray.indexOf(singlejsonPayload.eventName) == -1)
    {
      ...
      singlejsonPayload["@timestamp"] = date
      ...
      var send = new AWS.NodeHttpClient();
        ...
        send.handleRequest(req, null, function(httpResp) {
            ...
            httpResp.on('end', function (chunk) {
                    ...
                    console.log("--- START Send SNS Notification ---")
                    sendNotification(singlejsonPayload)
                    console.log("--- END Send SNS Notification ---")
            });
        }, function(err) {
...
function sendNotification(singlejsonPayload)
{ 
    var sns = new AWS.SNS();
    sns.publish({
        TopicArn: process.env.snsTopicArn,
        Message: JSON.stringify(singlejsonPayload)
    }, function(err, data) {
        if(err) {
